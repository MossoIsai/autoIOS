...
class Asistencia: UIViewController,UITableViewDelegate,UITableViewDataSource,MFMailComposeViewControllerDelegate,CNContactViewControllerDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailTextLabel?.font =  UIFont(name: "Arial-Regular", size: 17)
    ...
    func itemSeleccionado(indice: Int) {
        ...
        let action3 = UIAlertAction(title: "Contactar por Whatsapp", style: .default, handler: {
            ...
            let contactos = CNContact().mutableCopy() as! CNMutableContact
            contactos.givenName = "BWM Cuernvaca Used Car Center"
            contactos.nickname = "BMW Cuernavaca"
            ...
            let telefonoPrincipal = CNLabeledValue(label: CNLabelPhoneNumberMain,
                                                   value: CNPhoneNumber(stringValue: "5574260143"))
            let telefonoSecundario = CNLabeledValue(label: CNLabelWork,
                                                    value: CNPhoneNumber(stringValue: "7774283905"))
            
            contactos.phoneNumbers = [telefonoPrincipal,telefonoSecundario]
            
            let controller = CNContactViewController(forNewContact: contactos)
            //controller.delegate = self
            self.navigationController?.pushViewController(controller, animated:true)
            //NO SE PUEDE self.present(controller, animated: true, completion: nil)
        })
