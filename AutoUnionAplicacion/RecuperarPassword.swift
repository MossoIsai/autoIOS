...
class RecuperarPassword: UIViewController {
    ...
    @IBAction func btnEnviar(_ sender: Any) {
        ...
        if emailText.text == "" {
            alerta.showAlert(controller: self, titulo: "Llena el campo faltante", mensaje: "Ingresa un correo elextronico")
        }else{
            if self.validateEmail(texto: correo!){
                
            }else{
                alerta.showAlert(controller: self, titulo: "Correo invalido", mensaje: "Por favor verifica tu correo.")
            }
        }
